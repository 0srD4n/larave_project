            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex ">
                    @if(Auth::user()->status  < 3 )
                        ...
                        <x-nav-link :href="route('admin.control-panel')" :active="request()->routeIs('controlpanel')">
                            {{ __('Control Panel') }}
                        </x-nav-link>
                    @elseif(Auth::user()->status == 3 && Auth::user()->ortus)
                        <x-nav-link :href="route('ortu')" :active="request()->routeIs('ortu')">
                            {{ __('Dashboard') }}
                        </x-nav-link>
                    @else
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if(Auth::user()->status  > 3)
                ...
                <x-responsive-nav-link :href="route('admin.control-panel')" :active="request()->routeIs('controlpanel')">
                    {{ __('Control Panel') }}
                </x-responsive-nav-link>

            @elseif(Auth::user()->status == 3)
                <x-responsive-nav-link :href="route('ortu')" :active="request()->routeIs('ortu')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
            @else
            ...
            <x-responsive-nav-link :href="route('chatroom')" :active="request()->routeIs('chatroom')">
                {{ __('Chat Room') }}
            </x-responsive-nav-link>
        </div>
